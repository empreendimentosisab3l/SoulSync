// Script para fazer upload da imagem de IMC saudável para o Cloudinary
...
async function uploadHealthyImage() {
  try {
    console.log('🚀 Iniciando upload da imagem de IMC saudável...\n');

    // Path da imagem a ser enviada
    const imagePath = path.join(__dirname, 'public', 'images', 'quiz-v2', 'mid-sized.webp');

    if (!fs.existsSync(imagePath)) {
      console.error('❌ Imagem não encontrada em:', imagePath);
      console.log('\nPor favor, salve a imagem em: public/images/quiz-v2/mid-sized.webp');
      process.exit(1);
    }
    ...
    const result = await cloudinary.uploader.upload(imagePath, {
      ...
      public_id: 'card-11-mid-sized-healthy',
      use_filename: false,
