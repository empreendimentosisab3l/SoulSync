...
import { Timer, ArrowRight, Check, Lock, Star, AlertTriangle } from 'lucide-react';
...
export default function OfertaPage() {
    ...
    const [currentBmi, setCurrentBmi] = useState<number>(30.1); // Pre-calculated for 82kg/1.65m
    const [targetBmi, setTargetBmi] = useState<number>(25.0); // Pre-calculated for 68kg/1.65m
    const [weightLoss, setWeightLoss] = useState<number>(14); // 82 - 68
    const [openFaq, setOpenFaq] = useState<number | null>(0); // Primeiro FAQ aberto por padrão
    const [eventType, setEventType] = useState<string>('verao'); // Default event: "para o verão"
    ...
    const currentBodyFat = getBodyFatEstimate(currentBmi);
    const targetBodyFat = getBodyFatEstimate(targetBmi);
    ...
    const handleCheckout = () => {
        ...
        if (userData.name && userData.name !== 'Visitante') checkoutUrl.searchParams.set('name', userData.name);
            ...
            <div className="bg-[#8B0000] sticky top-0 z-50 border-b border-white/10 shadow-lg">
                <div className="container mx-auto px-4 py-3 flex items-center justify-between relative">
                    ...
                    <button
                        ...
                        className="bg-white text-[#8B0000] px-6 py-2.5 rounded-full font-bold text-sm sm:text-base hover:bg-gray-100 transition-colors shadow-md"
                    >
                        RECEBER MEU PLANO
                    </button>
                </div>
