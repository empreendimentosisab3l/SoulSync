...
export default function QuizV3Scratch() {
  const router = useRouter();

  useEffect(() => {
    pageview('/quiz-v3/scratch');
  }, []);
  ...
  const discountText = "no seu Plano de Hipnoterapia para Emagrecimento";
  const title = "Raspe para revelar seu desconto especial!";
  const subtitle = "Queremos que você comece sua jornada com uma agradável surpresa.";
  ...
  useEffect(() => {
    ...
    if (answers) {
      try {
        ...
        // Try to get name from any input field
        const nameAnswer = parsed['name'] || parsed['1'];
    ...
    <div className="min-h-screen bg-white flex items-center justify-center p-4">
      <div className="max-w-md w-full space-y-8 animate-fade-in">
        ...
        <div className={`relative transition-all duration-700 ${isRevealed ? 'scale-110 z-50' : 'scale-90'}`}>
          ...
          <div ref={couponContainerRef} className="bg-gradient-to-br from-teal-800 to-teal-600 rounded-3xl p-8 border-2 border-teal-500 shadow-2xl">
            <div className="text-center space-y-6">
              ...
              <div className="relative my-8">
                ...
                <div className="absolute -left-10 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white"></div>
                <div className="absolute -right-10 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white"></div>
              </div>

              {/* Coupon Code */}
              <div className="bg-white rounded-2xl p-6 shadow-lg border-4 border-dashed border-teal-300">
        ...
        {isRevealed && countdown !== null && countdown > 0 && (
          <div className="flex flex-col items-center justify-center animate-fade-in">
            <div className="relative">
              <div className="w-24 h-24 rounded-full bg-gray-100 border-4 border-teal-600 flex items-center justify-center animate-pulse">
        ...
        <div className={`transition-all duration-700 ${isRevealed ? 'blur-sm opacity-50 scale-95' : ''}`}>
          {!isRevealed && scratchPercentage > 0 && scratchPercentage < 30 && (
            <p className="text-center text-teal-600 text-sm animate-pulse">
          ...
          {!isRevealed && scratchPercentage >= 15 && (
            <button
              onClick={revealAll}
              className="w-full bg-gray-100 border-2 border-teal-300 text-gray-900 py-3 rounded-full font-medium hover:bg-teal-50 transition-all duration-300 animate-fade-in"
            >
